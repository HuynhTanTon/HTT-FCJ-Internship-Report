...
Kiến trúc chọn **Amazon S3** (Static Website Hosting) + **AWS Lambda Function URL** + **Amazon DynamoDB**, triển khai tại region **ap-southeast-1 (Singapore)**. Không dựng EC2, không dùng API Gateway — Lambda Function URL đóng vai trò endpoint HTTPS công khai cho API.
...
| Data | DynamoDB `url-shortener-links` | Lưu `shortCode` ↔ `originalUrl`, `clickCount` |
...
3. [Triển khai Backend](5.3-Backend/) — Lambda, Function URL, kiểm thử API.
4. [Triển khai Frontend](5.4-Frontend/) — S3 hosting, Bucket Policy, upload và e2e.
5. [Nâng cao](5.5-Nang-cao/) — đếm click atomic và cảnh báo CloudWatch.
6. [Dọn dẹp](5.6-Don-dep/) — thu hồi tài nguyên sau demo.
